...
/*设置推送的引擎 xiaoMi||huaWei
  示例：MixPushPlugin.setPushEngine(["xiaoMi"],successCallback,errorCallback);
 */
...
示例：let miId = '288230*****896';
      let miKey = '5901***96';
      MixPushPlugin.registerPush([miId, miKey],successCallback,errorCallback);
 */       
...
/*退出推送
  示例：MixPushPlugin.exitPush([],successCallback,errorCallback);
*/
...
/*为指定用户设置userAccount
  示例：MixPushPlugin.setAccount(["dmc"],successCallback,errorCallback);
*/
...
/*取消指定用户的userAccount。
  示例：MixPushPlugin.unsetAccount(["dmc"],successCallback,errorCallback);
*/
...
/*获取客户端的RegId。
  示例：MixPushPlugin.getRegId([],(id)=>{alert(id);}),errorCallback);
*/
...
/*为指定用户设置alias
  示例：MixPushPlugin.setAlias(["123"],successCallback,errorCallback);
*/
...
/*取消指定用户的alias
  示例：MixPushPlugin.unsetAlias(["123"],successCallback,errorCallback);
*/
...
/*为某个用户设置订阅topic
  示例：MixPushPlugin.subscribe(["112"],successCallback,errorCallback);
*/
...
/*取消某个用户的订阅topic
  示例：MixPushPlugin.unsubscribe(["112"],successCallback,errorCallback);
*/
...
/*暂停接收MiPush服务推送的消息
  示例：MixPushPlugin.pausePush([],successCallback,errorCallback);
*/
...
/*恢复接收MiPush服务推送的消息，这时服务器会把暂停时期的推送消息重新推送过来
  示例：MixPushPlugin.resumePush([],successCallback,errorCallback);
*/
...
/*关闭MiPush推送服务，不再维护长连接，服务不可用
  示例：MixPushPlugin.disablePush([],successCallback,errorCallback);
*/
...
/*打开 MiPush 推送服务，继续维护长连接
  示例：MixPushPlugin.enablePush([],successCallback,errorCallback);
*/
...
/*清除小米推送弹出的通知
  示例：MixPushPlugin.clearNotification([],successCallback,errorCallback);
*/
...
/*设置APP角标数字
  示例：设置数字20当设置0的时候就等同于badgerRemoveCount，MixPushPlugin.badgerApplyCount([20],successCallback,errorCallback);
 */
...
//清空APP角标数字
//示例：MixPushPlugin.badgerRemoveCount([],successCallback,errorCallback);
